...
export const LampContainer = ({
  ...
  return (
    <div
      className={`relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-brand-red w-full rounded-md z-0 pt-24 ${className}`}
    >
      {/* Sistema de luces centrado y desplazado hacia abajo */}
      ...
      {/* AQUÍ ESTÁ LA MAGIA: 
          Cambiamos el agresivo -translate-y-48 por un valor mínimo (-translate-y-8).
          Esto permite que el texto baje a su posición natural y no choque con la navegación.
      */}
      <div className="relative z-50 flex -translate-y-8 flex-col items-center px-5 pb-20 w-full">
        {children}
      </div>
    </div>
  );
